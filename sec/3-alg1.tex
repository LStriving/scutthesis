...
目前，视频透视吞咽检查（VFSS）被公认为评估吞咽障碍的“金标准”，
...
微动作的精确定位依赖于对关键解剖结构（如舌骨）微小运动的持续追踪。
然而，X 射线成像中成像对比度低 \cite{zheng2004automated, amiot2016spatio} ，关键结构轮廓模糊。
此外，结构性遮挡经常致使关键轮廓不可见（如下颌骨对舌骨的遮挡），从而降低了动作识别的可靠性。
2) \textbf{吞咽微动作的瞬时性}：
...
它们主要依赖于外观特征（即 RGB 图像和光流），极易受到噪声和背景干扰的影响。
...
因为它们无法解决噪声外观特征与稀疏骨架热图之间的语义对齐问题。
...
为解决上述挑战，本文提出了\fullname (\textbf{\sexyname})。
该框架旨在增强VFSS场景下TMAL任务的时空建模能力。
基于临床先验知识，解剖区域（如软腭、椎骨和舌骨）的关键点对于量化吞咽运动学特征至关重要\cite{goldfield2010premature,zhang2021automatic,hsiao2023deep}。
...
与外观数据不同，这种表征天然地过滤了无关的背景细节，从而能够显式追踪具有临床意义的运动以实现精确定位。

随着骨架模态的引入，如何实现模态间的高效交互成为关键。
为了有效集成这些模态，设计了一个双分支架构来并行处理外观特征和骨架特征。
我们提出了\crossblockfull（\crossblockshort）模块来促进两个分支间的特征层面上的相互校准。
具体而言，为解决 X 射线图像中的空间模糊性，该模块利用骨架轨迹作为空间置信度门控，帮助模型关注吞咽过程重要的区域来动态校准外观特征。
此外，我们还采用了一个\channelblockfull（\channelblockshort），显式地减去各通道间共享的静态背景信息，从而显著增强特征的区分度。
...
借鉴 Ruan 等人~\cite{ruan2023temporal} 的思路，
本文采用“由粗到细”（Coarse-to-Fine）的策略以降低微动作定位的难度并提升模型性能。
...
针对精细定位阶段，本文引入骨架热图作为一种编码了解剖学先验的新模态。
通过追踪八个关键点构建结构化骨架序列，
我们增强模型对解剖结构的关注度并抑制背景噪声，克服了仅依赖外观数据的局限性。
...
本文通过构建骨架关键点序列来实现解剖先验的集成。
具体而言，本文参考了吞咽分析中运动学参数分析中常利用的结构~\cite{zhang2021automatic, hsiao2023deep, goldfield2010premature}，
...
最终的热图序列通过在时间维度堆叠处理后的 2D 热图构建而成。
...
\({\rm SiLU}\) 为 Sigmoid 线性单元激活函数 \cite{elfwing2018sigmoid}，\({\rm Conv}\) 为一维深度卷积，\(\psi\) 为线性投影层，\(\odot\) 表示逐元素乘法（Hadamard Product），\({\rm flip}\) 表示沿时间维度的翻转操作。
...
本章方法嵌入了一个新颖的 \channelblockfull(\channelblockshort) 模块，从而构建了完整的 \crossblockshort：
\begin{equation}
...
\multirow{2}{*}{设置}    &\multicolumn{5}{c}{mAP@tIoU 阈值} & \multirow{2}{*}{\makecell{参数量\\(M) }}& \multirow{2}{*}{\makecell{FLOPs\\(G)}} \\
  &  0.1&0.3 & 0.5 & 0.7 & Avg. & & \\
\midrule
w/o \channelblockshort  & 82.3&73.3  & 58.1  & 35.1 & 63.3  & 109.1& 116.9\\
\midrule
\channelblockshort-Add  & 81.7&73.3  &  58.6  & 36.0 &   63.5   & 109.2& 116.9\\
...
相比之下，本文提出的 \sexyname 取得了 64.3\% 的平均 mAP，全面超越了上述三类方法：相较于最先进的 TAL 方法 (ActionMamba) 提升了 \textbf{5.3\%}；
相较于现有的 VFSS 特定优化策略 (Ruan \etal) 提升了 \textbf{14.4\%}；
相较于骨架增强基线 (Hyder \etal) 提升了 \textbf{5.0\%}。
值得注意的是，本文提出的框架具有良好的架构通用性，不局限于特定的检测器。
...
为验证模型的泛化能力与临床鲁棒性，本文引入了采集自不同医疗中心（明心康复中心）的独立外部数据集进行测试。
...
相较于简单的双分支基线模型，这 2.2\% 的增益有力地证明了 \crossblockshort 提供了有效的特征校准，
而非仅仅进行单纯的模态集成。
...
\textbf{融合模块性能对比}\quad 表~\ref{tab: CCM ablation} 对比了不同交互机制的效果。“Self-Mamba”在公式~\ref{eq: ccm} 中对单一模态执行自注意力计算（而非跨模态交互），实验结果显示其性能低于双分支基线。尽管交叉注意力机制（Cross-Attention）相较于基线带来了微小的提升（mAP@Avg. +0.3\%），但本文提出的 \crossblockshort 实现了显著的性能飞跃（mAP@Avg. 提升 \textbf{2.2\%}）。
...
尽管所有方法均能成功检出微动作，但本文方法在时序边界的回归精度上表现更为出色，预测结果与真值更加贴合。
...
相比之下，本文框架的检测结果与标签值保持了高度重叠，准确覆盖了所有关键动作。
...
针对 VFSS 影像中常见的噪声干扰与结构模糊问题，本文通过将具有临床意义的解剖学先验编码为骨架热图，
