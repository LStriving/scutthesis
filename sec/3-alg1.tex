...
\section{引言}
吞咽障碍（Dysphagia）~\cite{rommel2016oropharyngeal} 常见于儿童和老年人，影响超过 30\% 的 65 岁及以上人群 \cite{lee2019automatic}。它也是多种疾病的早期症状，包括帕金森病 \cite{pflug2018critical} 和食管癌 \cite{hughes2011management}。对高危人群进行早期且准确的吞咽功能评估对于及时干预至关重要。
评估吞咽障碍的“金标准”是视频透视吞咽检查（VFSS），它捕捉吞咽过程中口腔期、咽期和食管期的动态 X 射线图像。
在 VFSS 中，对亚秒级微动作（例如舌骨运动）进行时序定位对于检测异常至关重要 \cite{molfenter2012temporal, kendall2000timing}。
获取时间参数（即微动作的开始/结束时间）正是微动作定位任务的目标。

由于成像和吞咽生理学的两个固有特征，VFSS 中的时序微动作定位（TMAL）比日常活动定位要困难得多：
1) 解剖结构跟踪的空间模糊性：微动作的精确定位需要跟踪关键解剖结构（例如舌骨）的细微运动。
然而，X 射线成像中不可避免的背景噪声 \cite{zheng2004automated, amiot2016spatio} 以及无关解剖区域的遮挡可能会损害关键轮廓的可见性，降低动作识别的可靠性，并可能影响临床判断。
2) 吞咽微动作的时间短暂性：微动作持续时间极短，有时仅为 0.3 秒 \cite{ruan2023temporal}，这使得精确的时间边界定位充满挑战。

目前既定的工作流程主要通过专家的逐帧人工标注来完成这项任务 \cite{kellen2010computer}，这不仅劳动密集，而且容易产生观察者间的差异。为了减轻这一负担，自动化方法应运而生 \cite{ruan2023temporal, jeong2023application}，它们通常通过在定位前检测整体吞咽事件 \cite{ruan2023temporal} 或裁剪视频以关注相关片段 \cite{jeong2023application} 来处理短暂的微动作。
尽管在处理短暂动作方面取得了进展，但这些方法仍存在局限性。具体而言，它们单纯依赖基于外观的数据（即 RGB 和光流），使其容易受到噪声和背景干扰的影响。例如，如图~\ref{fig: motivation}(a) 上方子图所示，作为跟踪微动作的关键解剖标志，舌骨在 RGB 帧中常因噪声或下颌等附近无关结构的遮挡而变得模糊不清。
自然地，显式引入解剖结构先验可以帮助缓解这一问题。然而，有效利用此类先验仍具挑战性。首先，关于如何有效且稳健地表示解剖结构尚未达成共识。其次，将解剖引导与外观特征整合以进行高效的时空建模并非易事。简单的策略（如特征拼接或基本的后期融合，例如平均预测结果）并非最优解，正如我们在第 \ref{sec: ablation} 节的消融实验中所证明的那样。% (Tab. \ref{tab: modality ablation})
...
针对上述问题，我们提出了 \fullname (\textbf{\sexyname})，这是一个旨在增强 VFSS 中 TMAL 时空建模的新颖框架。
借鉴既有的临床经验 \cite{goldfield2010premature,zhang2021automatic,hsiao2023deep}，我们发现来自解剖区域（例如软腭、椎骨和舌骨）的关键点对于测量吞咽运动学至关重要。据此，我们首先引入“骨架”\footnote{本文中的“骨架”一词指由选定的解剖关键点（例如舌骨、椎骨角、软腭）形成的结构表示，而非传统的人体骨骼系统。}模态以增强解剖学表示，如图~\ref{fig: motivation}(b) 所示，其中与吞咽微动作相关的关键点被标注、连接并编码为热图序列。与外观数据不同，骨架热图过滤了无关的背景细节，并改善了对吞咽评估中具有临床意义的细微运动的跟踪。
引入骨架模态后，模态间的交互变得至关重要。虽然交叉注意力（Cross-Attention）~\cite{vaswani2017attention} 是一个常见的选择，但其二次方复杂度限制了可扩展性。最近，Mamba \cite{gu2023mamba} 提供了具有线性复杂度的高效序列建模能力，但其在跨模态交互中的应用鲜有探索。为了结合两者的优势，我们提出了一个 \crossblockfull (\crossblockshort) 模块，旨在桥接外观特征和骨架特征以进行时空建模。此外，我们在 \crossblockshort 内部引入了一个 \channelblockfull(\channelblockshort) 模块，通过抑制跨通道的冗余信息来自适应地优化特征。通过堆叠多个 \crossblockshort 模块，模型逐步优化多模态特征以实现时序定位。

\section{问题定义与方法概述}

% \textbf{Problem definition.}
时序微动作定位与识别任务旨在预测微动作的开始和结束时间,并识别动作的类别。
给定一个未裁剪的吞咽视频 $V$, 
设标签 $Z=\{z_1, z_2, \dots, z_{N}\}$ 表示视频中的一组微动作实例，其中 $N$ 代表实例的数量，因视频而异。
每个实例 $z_i=(s_i, e_i, a_i)$ 由其开始时间 $s_i$、结束时间 $e_i$ 和动作类别标签 $a_i$ 定义，
其中 $a_i \in \{1, \dots, C\}$（$C$ 为预定义的微动作类别数量），且 $s_i < e_i$。

这项任务具有挑战性：在时间上，微动作持续时间极短且相互重叠；在空间上，图像中的细微运动、背景噪声和模糊轮廓阻碍了关键解剖结构的识别和精确的动作定位。

为了解决这些挑战，我们提出了一个名为 \fullname (\textbf{\sexyname}) 的多模态融合框架。遵循Ruan等人~\cite{ruan2023temporal} 的方法，我们采用由粗到细的定位策略来简化微动作定位并获得更好的性能。具体而言，我们首先在粗略阶段使用现成的检测器（如 ActionFormer~\cite{zhang2022actionformer} 或 ActionMamba\cite{chen2024video}）获取整个吞咽动作的候选区域（proposals），然后在精细定位阶段，通过将候选区域的质心向外扩展固定的持续时间（例如 4 秒）来裁剪视频。粗略阶段的标签为 $Z^{1}=\{z^1_1,z^1_2,\ldots,z^1_M\}$，其中 $M = \frac{N}{C}$ 代表吞咽实例的总数。每个 $z^1_i=(s_i,e_i)$ 由整个吞咽事件的开始时间 $s_i$ 和结束时间 $e_i$ 定义。第二阶段的标签表示为 $Z^2 = \{z^2_1, \dots, z^2_C\}$。粗略阶段是一个简单的二分类时序动作定位任务，不是本文的重点，因此不再赘述。
...
\textbf{外观表示} 视频片段的外观特征是使用预训练的 I3D 模型 \cite{carreira2017quo} 提取的，遵循 \cite{ruan2023temporal} 的方法。外观特征可以表示为：
...
\textbf{模型推理的后期融合} 在推理阶段，我们从外观和骨架分支获得预测结果。预测的开始时间 $\hat{s}_t$、结束时间 $\hat{e}_t$ 和类别概率 $\hat{p}_t$ 计算如下：
...
\section{实验结果}
\textbf{数据集：}
我们使用了来自文献 \cite{ruan2023temporal} 的造影视频数据集，该数据集收集自中山大学附属第三医院。数据集包含来自 71 名受试者（25 名男性，46 名女性）的 847 个视频（总时长：508 分钟）。数据集按受试者以 4:1:1 的比例划分为训练集、验证集和测试集。它包含了七种吞咽微动作和完整吞咽过程的时序标注。每个微动作的平均持续时间总结在表~\ref{tab:avg_duration} 中。
...
\textbf{关键点标注：} 我们从 210 个裁剪后的吞咽视频中提取了 13,895 帧。这些数据被划分为三个子集用于关键点定位：8,302 张图像（来自 131 个视频）用于训练，1,518 张图像（来自 16 个视频）用于验证，以及 4,075 张图像（来自 63 个视频）用于测试。标注工作由专家使用开源的 \textit{CVAT} 工具\footnote{https://github.com/cvat-ai/cvat} 完成。为了确保可靠性，我们采用了多步验证流程，包括标注者之间的交叉验证和一致性检查。}
...
\textbf{实施细节：}
我们使用 HRNet~\cite{wang2020deep} (HRNet-W32) 进行骨架提取，并使用相同的训练策略。我们采用 VideoMamba \cite{li2025videomamba}（576 维特征）进行端到端的骨架特征提取，结合 3 层我们的通道增强型 Cross-Mamba 以增强外观特征和骨架特征之间的交互。对于下游任务，我们遵循 ActionMamba \cite{chen2024video} 的设置，但移除了主干网络的最后一层，因为它是针对长时间动作的，对微动作定位没有贡献。优化器设置、学习率、权重衰减和预热策略保持不变。遵循 Ruan 等人\cite{ruan2023temporal}，我们采用由粗到细的定位机制和针对 VFSS 的单类别分类。在粗略阶段，为了公平比较，我们利用相同的预训练 I3D \cite{carreira2017quo} 来提取外观特征。在精细定位阶段，所有模型均以 batch size 为 16 训练 45 个 epoch。关于超参数，公式~\ref{eq: heatmap_k} 和公式~\ref{eq: heatmap_l} 中的标准差 $\sigma$ 设为 4，公式~\ref{eq: ccm} 中 $\gamma$ 的初始值设为 $10^{-5}$，公式~\ref{eq: late fusion} 中的融合权重 $\omega$ 设为 0.6。第~\ref{sec:hyper} 节研究了超参数的选择。
...
如表~\ref{tab: external} 所示，我们的方法（使用 ActionMamba 作为检测器）在大多数 tIoU 阈值上都取得了更好的性能。平均 mAP 比之前的 VFSS 方法提高了 9.0\%，比骨架增强方法提高了 5.9\%，证明了增强的时序定位鲁棒性。这些结果证实了我们的方法对真实临床环境中异构采集条件和不同患者特征具有优越的泛化能力。为了支持临床应用，我们开发了一个用于 VFSS 视频分析的智能原型系统。这一实际实现展示了该方法在现实场景中的可用性。

\subsection{消融实验}
\label{sec: ablation}
我们评估了 \sexyname 中关键组件的效果，报告 mAP@0.1、mAP@0.3、mAP@0.5、mAP@0.7 和 mAP@Avg.（0.1 到 0.7）。

\textbf{多模态建模的消融} 如表~\ref{tab: modality ablation} 所示，与 \cite{ruan2023temporal} 中的发现一致，采用单类别分类的由粗到细定位策略优于单阶段多类别分类方法（mAP@Avg. 59.0\% → 61.7\%）。关于模态的影响，仅使用骨架分支表现欠佳，因为像 UES 开放这样的微动作很难仅从关键点运动中推断出来。外观分支表现较好，但对视觉噪声（例如，舌骨缺失）敏感。对编码器特征的简单拼接阻碍了优化过程，且未能有效利用骨架数据。使用双分支架构并进行后期融合（即，平均预测结果）略微提高了性能。相比之下，我们的 \sexyname 结合了 CCM 模块，利用了先验知识并收集了有效的表示，将 mAP@Avg. 从 62.1\% 提升至 64.3\%。
...
\textbf{不同融合模块的比较} 在表~\ref{tab: CCM ablation} 中，“Self-Mamba”在公式~\ref{eq: ccm} 中使用相同的模态（而不是互补模态）作为输入，导致性能略有下降。虽然交叉注意力（Cross-Attention）相较于基线略微改善了结果（mAP@Avg. +0.3\%），但我们的 \crossblockshort 带来了显著的性能提升（mAP@Avg. \textbf{+2.2\%}）。
...
\textbf{\crossblockshort 中 \channelblockshort 模块的消融。} 移除通道增强（w/o CE）会导致显著的性能下降（mAP@Avg. -1.0\%），证实了其必要性。虽然 SE\cite{hu2018squeeze} 略微改善了结果，但我们的 \channelblockshort 在增强变体中以最低的参数量（109.2M）和有限的额外成本实现了最佳精度（64.3\% mAP@Avg.）。关键是，\channelblockshort 中的减法操作比加法变体（CE-Add）的平均 mAP 高出 +0.8\%，凸显了冗余抑制相对于特征强化的有效性。

\textbf{骨架分支不同骨架编码器的消融}
...
\textbf{骨架分支不同输入的消融} 我们评估了使用不同热图输入的骨架分支的性能：仅关键点、仅线和骨架热图。使用 VideoMamba 进行特征提取。如表~\ref{tab: heatmap input ablation} 所示，骨架热图取得了最佳性能，其次是仅线热图，而关键点热图结果最差。结果证明了结合关键点和线热图的益处。
...
\textbf{错误分析}
% To analyze the limitations of our method, we perform a quantitative error breakdown of false positives across various tIoU thresholds (0.1 to 0.7), as shown in Fig.~\ref{fig: false-positive}. Our method achieves a high true positive rate on Top-1G predictions and limited localization error. The most dominant error type is ``Background Error", where the model incorrectly identifies irrelevant temporal regions as micro-actions. Similar to observations in BasicTAD \cite{yang2023basictad} and ViT-TAD \cite{yang2024adapting}, this is mainly due to the limited regression anchors.
为了分析我们方法的局限性，我们对不同 tIoU 阈值（0.1 到 0.7）下的误报进行了定量错误细分，如图~\ref{fig: false-positive} 所示。
我们的方法在 Top-1G 预测上实现了较高的真正率，并且定位误差有限。最主要的错误类型是“背景错误”，即模型错误地将无关的时间区域识别为微动作。
与 BasicTAD \cite{yang2023basictad} 和 ViT-TAD \cite{yang2024adapting} 中的观察结果类似，这主要是由于回归锚点的限制。
此外，“定位错误”占次要比例，表明尽管我们的框架在时序边界检测方面表现出色，但在某些情况下仍然难以精确捕捉微动作的开始和结束时间。
这些见解强调了未来改进的潜在方向，例如增强锚点设计和引入更精细的边界调整机制，以进一步提升微动作定位的准确性。
